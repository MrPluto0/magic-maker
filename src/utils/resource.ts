...
export class ResourceFactory {
	...
	static async createFromFile(file: File): Promise<Resource> {
		...
		try {
			if (file.type.startsWith("video/")) {
				return ResourceFactory.createVideoResource(file, baseResource);
			} else if (file.type.startsWith("audio/")) {
				return ResourceFactory.createAudioResource(file, baseResource);
			} else if (file.type.startsWith("image/")) {
				return ResourceFactory.createImageResource(file, baseResource);
			} else if (file.type.startsWith("text/")) {
				return ResourceFactory.createTextResource(file, baseResource);
			} else {
	...
	private static validateFile(file: File): void {
		...
		if (maxSize && file.size > maxSize) {
			throw new Error(`文件大小超过限制 (${ResourceFactory.formatFileSize(maxSize)})`);
		}
