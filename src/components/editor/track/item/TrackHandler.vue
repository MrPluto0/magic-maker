...
function initLimits(lineData: TrackItem[], trackItem: TrackItem) {
  ...
  const limitData = {
    ...
    maxEnd: afterTrack ? afterTrack.start : baseFps * 60 * 60, // 最长一小时
  };
  if (isVA) {
    // 音视频结尾受资源大小限制
    ...
    limitData.maxEnd = afterTrack
      ...
      : Math.min(rightMaxWidth + limitData.start, baseFps * 60 * 60);
...
function setTrackFrameData(frameCount: number, handleType: string) {
  ...
  if (handleType === "left") {
    ...
    if (isVA) {
      ...
      store.trackList[props.lineIndex].list[props.itemIndex].offsetL = Math.max(
        offsetL + diffStart,
        0
      );
    } else {
